...
The routes are configured in app/config/routes.rb
  ...
  router.draw do |route|
	...
	#
	# # Resource Example
	# route.for '/users/:id', :to => 'users#show' # :get is implied
	# route.for '/users/:id', :to => 'users#create',  :methods => [:post]
	# route.for '/users/:id', :to => 'users#update',  :methods => [:put]
	# route.for '/users/:id', :to => 'users#destroy', :methods => [:delete]
  end
...
Fastr currently has support for HAML and eRuby templates. Any instance variables in your controller are available in the template. The correct template engine will be chosen based on the file extension. By default no template engines will be loaded, you need to explicitly require the engine(s) you will be using in your config.ru file (below the require 'fastr' line):
